...
import { DEFAULT_LM_STUDIO_BASE_URL, DEFAULT_LM_STUDIO_MODEL } from "../src/inference/loadConfig.js";
...
async function main(): Promise<void> {
  ...
  const body = {
    model: process.env.LM_STUDIO_MODEL ?? DEFAULT_LM_STUDIO_MODEL,
    messages: [
      {
        role: "user",
        content:
  ...
  const baseUrl = (process.env.LM_STUDIO_BASE_URL ?? DEFAULT_LM_STUDIO_BASE_URL).replace(/\/$/, "");
